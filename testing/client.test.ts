...
describe("Client", () => {
  describe("query", () => {
    it("returns a builder with send and use", () => {
      const client = new Client({ transport: new MemoryTransport(() => ({ data: null })).send });
    ...
    it("returns { ok: true } with data on success", async () => {
      ...
      const client = new Client({ transport: new MemoryTransport(() => ({ data })).send });
    ...
    it("passes variables to the transport", async () => {
      ...
      const client = new Client({ transport: transport.send });
